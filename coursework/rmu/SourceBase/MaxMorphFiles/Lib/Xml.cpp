...
bool AttrArray::Exists(const String& AttrName, int& AttrIndex) const {
...
bool IsWowel(char a) {
    return a == 'à' || a == 'å' || a == '¸' || a == 'è' || a == 'î' || a == 'ó' || a == 'û' || a == 'ý' || a == 'þ' || a == 'ÿ';
}
...
void XMLElement::ParseTag(const String& Tag, XMLElem* xmlElem) {
    int Pos;
    int length;
    bool Quoted;
    bool LastQuot;
...
XMLElem::XMLElem() : attrs(new AttrArray),
                     elems(new XMLElems) {}

XMLElem::XMLElem(const String& Name) : attrs(new AttrArray),
                                       elems(new XMLElems),
                                       name(Name) {}
