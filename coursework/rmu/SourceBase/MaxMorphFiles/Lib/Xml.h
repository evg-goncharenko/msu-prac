...
struct Attr {
    ...
    String Value;
    Attr(){};
    Attr(const String& name, const String& value) : Name(name),
                                                    Value(value) {}
    Attr(const Attr& Attr) : Name(Attr.Name),
                             Value(Attr.Value) {}
};
...
public:
    ...
    AttrArray(){};
    AttrArray(const AttrArray& AttrArray) : Array(AttrArray.Array){};
...
public:
    // Life-cycle
    XMLText() {}
    XMLText(const String& Line) : text(Line) {}
    XMLText(const XMLText& aXMLText) : text(aXMLText.text) {}
    ~XMLText() {}
    // Operations
