...
bool
FileConfig::Open ( const String & CfgFileName )
{ 
    String Line;
    String Name;
    String Value;
    ...
    if ( CfgFile.Open ( CfgFileName, FileStream::modeRead ) )
    {
        ...
        while ( CfgFile.ReadLine ( Line ) )
        {
            ...
            {
                RemarkPos = Line.Find ( "#" );
                if ( RemarkPos == 0 ) Line.Clear ();
                else if ( 0 < RemarkPos ) Line = Line.SubStr ( 0, RemarkPos );
...
bool
FileConfig::Read  ( const String & AttrName, String& Value )
{
    int Position;
