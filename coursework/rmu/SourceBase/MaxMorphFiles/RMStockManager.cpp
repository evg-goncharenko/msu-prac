...
void CRMStockManager::BindFlexLinks(const String& FlexID, CRMParadigm* PClass) {
    CRMFlex* Flex;
    bool YetNotLinked;
    int i;
...
CRMStockManager::FindPClasses(const String& FlexID, const String& SClass) const {
    ...
    if ((Flex = FlexLinks[FlexID])) {
        for (Index = 0; Index < Flex->PClasses.Count(); ++Index) {
            ...
            if (SClass == "personalpronoun")
                Checked = (ID.Find("PersPr") == 0);
            else if (SClass == "reflexivepronoun")
                Checked = (ID.Find("ReflPr") == 0);
            else if (SClass == "pronoun")
                Checked = (ID.Find("Pron") == 0);
            else if (SClass == "pronounadjective")
                Checked = (ID.Find("PrAdj") == 0);
            else if (SClass == "adjective")
                Checked = (ID.Find("Adj") == 0);
            else if (SClass == "noun")
                Checked = (ID.Find("Noun") == 0);
            else if (SClass == "verb")
                Checked = (ID.Find("Verb") == 0);
...
void CRMStockManager::LoadFromFile(const String& BaseName, IRMStockManager::FileFormat Format) {
    try {
        // Start job (busy)
        BusyStatus = 99;
...
void CRMStockManager::LoadLexemsFromBinaryFile(const String& BaseName) {
    //int         AttrIndex  ( 0 );
    int LineNumber(0);
    String Line;
...
void CRMStockManager::LoadLexemsFromXMLFile(const String& BaseName) {
    int AttrIndex(0);
    String Line;
    XMLElement* XML = NULL;
    String eofmsg("Unexpected end of file");
                    ...
                    if (XML->IsElem() && 1 < XML->Attrs()->Count()) {
                        ...
                        if (XML->Name() == ERMClassNames[ERM_PERSONAL_PRONOUN][1]) {
                            ...
                            for (AttrIndex = 0; AttrIndex < XML->Attrs()->Count(); ++AttrIndex) {
                                ...
                                if (XML->AttrName(AttrIndex) == "animate") {
                                    if (XML->Attr(AttrIndex) == "yes")
                                        PersonalPronoun->Animate = 0;
                                    else if (XML->Attr(AttrIndex) == "no")
                                        PersonalPronoun->Animate = 1;
                                    else
                                        PersonalPronoun->Animate = 2;
                                }
                                // Gender
                                if (XML->AttrName(AttrIndex) == "gender") {
                                    if (XML->Attr(AttrIndex) == "male")
                                        PersonalPronoun->Gender = 0;
                                    else if (XML->Attr(AttrIndex) == "female")
                                        PersonalPronoun->Gender = 1;
                                    else if (XML->Attr(AttrIndex) == "neuter")
                                        PersonalPronoun->Gender = 2;
                                    else
                                        PersonalPronoun->Gender = 3;
                                }
                                // Number
                                if (XML->AttrName(AttrIndex) == "number") {
                                    if (XML->Attr(AttrIndex) == "single")
                                        PersonalPronoun->Number = 0;
                                    else if (XML->Attr(AttrIndex) == "plural")
                                        PersonalPronoun->Number = 1;
                                    else
                                        PersonalPronoun->Number = 2;
                                }
                                // Person
                                if (XML->AttrName(AttrIndex) == "person") {
                                    if (XML->Attr(AttrIndex) == "1")
                                        PersonalPronoun->Person = 0;
                                    else if (XML->Attr(AttrIndex) == "2")
                                        PersonalPronoun->Person = 1;
                                    else
                                        PersonalPronoun->Person = 2;
                                }
                                // Paradigm class
                        ...
                        // 3-2
                            ...
                            for (AttrIndex = 0; AttrIndex < XML->Attrs()->Count(); ++AttrIndex) {
                                ...
                                if (XML->AttrName(AttrIndex) == "animate") {
                                    if (XML->Attr(AttrIndex) == "yes")
                                        ReflexivePronoun->Animate = 0;
                                    else if (XML->Attr(AttrIndex) == "no")
                                        ReflexivePronoun->Animate = 1;
                                    else
                                        ReflexivePronoun->Animate = 2;
                                }
                                // Gender
                                if (XML->AttrName(AttrIndex) == "gender") {
                                    if (XML->Attr(AttrIndex) == "male")
                                        ReflexivePronoun->Gender = 0;
                                    else if (XML->Attr(AttrIndex) == "female")
                                        ReflexivePronoun->Gender = 1;
                                    else if (XML->Attr(AttrIndex) == "neuter")
                                        ReflexivePronoun->Gender = 2;
                                    else
                                        ReflexivePronoun->Gender = 3;
                                }
                                // Number
                                if (XML->AttrName(AttrIndex) == "number") {
                                    if (XML->Attr(AttrIndex) == "single")
                                        ReflexivePronoun->Number = 0;
                                    else if (XML->Attr(AttrIndex) == "plural")
                                        ReflexivePronoun->Number = 1;
                                    else
                                        ReflexivePronoun->Number = 2;
                                }
                                // Paradigm class
                                if (XML->AttrName(AttrIndex) == "paradigmid")
...
void CRMStockManager::LoadPClassesFromBinaryFile(const String& BaseName) {
...
void CRMStockManager::LoadPClassesFromXMLFile(const String& BaseName) {
    int i, j, AttrIndex;
    XMLElement* XML = NULL;
...
void CRMStockManager::LoadStemsFromBinaryFile(const String& BaseName) {
...
void CRMStockManager::LoadStemsFromXMLFile(const String& BaseName) {
...
void CRMStockManager::LoadSchemasFromBinaryFile(const String& BaseName) {
                                    ...
                                    if (Stream.ReadLine(Line)) {
                                        ...
                                        while (0 < StressCount && Stream.GetState() == IStreamBase::STREAM_OK) {
                                            ...
                                            if (Line.IsEmpty())
                                                ...
                                                Stress.Pos = Line.ToInt();
                                            ++LineNumber;
                                            Stream.ReadLine(Line);
                                            if (Line.IsEmpty())
                                                Stress.Aux = 0;
                                            else
                                                Stress.Aux = Line.ToInt();
                                            ++LineNumber;
...
void CRMStockManager::LoadSchemasFromXMLFile(const String& BaseName) {
    int i, j, k;
                                    ...
                                    for (j = 0; j < XML->Child(i)->Children()->Count(); ++j) {
                                        ...
                                        if (nestedXML->Name() == "flex") {
                                            ...
                                            for (k = 0; k < nestedXML->Children()->Count(); ++k) {
                                                Stress.Pos = Stress.Aux = 0;
                                                Stress.NestedID.Clear();
...
void CRMStockManager::SaveToFile(const String& BaseName, enum IRMStockManager::FileFormat Format) {
    try {
        if (Format == SM_FORMAT_BINARY) {
...
void CRMStockManager::SaveLexemsToBinaryFile(const String& BaseName) {
    int ItemIndex;
    String Line;
    // Lexem elements
    FileStream File;
    ...
    if (Lexems.Count()) {
        try {
            ...
            for (ItemIndex = 0; ItemIndex < Lexems.Count(); ++ItemIndex) {
                ...
                if (Lexem->Paradigm)
                    Stream.WriteLine(Lexem->Paradigm->UID);
                else
                    Stream.WriteLine("");
                if (Lexem->StressSchema)
                    Stream.WriteLine(Lexem->StressSchema->UID);
                else
                    Stream.WriteLine("");
...
void CRMStockManager::SaveLexemsToXMLFile(const String& BaseName, int DicType) {
    int ItemIndex;
    FileStream File;
    String Line;
...
void CRMStockManager::SavePClassesToBinaryFile(const String& BaseName) {
...
void CRMStockManager::SavePClassesToXMLFile(const String& BaseName, int DicType) {
    int ItemIndex, AttrIndex, SubIndex;
...
void CRMStockManager::SaveSchemasToBinaryFile(const String& BaseName) {
...
void CRMStockManager::SaveSchemasToXMLFile(const String& BaseName, int DicType) {
...
void CRMStockManager::SaveStemsToBinaryFile(const String& BaseName) {
...
void CRMStockManager::SaveStemsToXMLFile(const String& BaseName, int DicType) {
    int ItemIndex, AttrIndex;
    FileStream StemFile;
    String Line;
...
void CRMStockManager::UpdateCounters(bool Clear) {
    unsigned long i;
