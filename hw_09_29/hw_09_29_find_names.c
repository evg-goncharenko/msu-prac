/*
    The program reads the text of C program and prints in alphabetical order all groups of variable
    names in which the first n=6 characters match, but the subsequent ones differ in some way.
    Does not process the inside of quoted strings and comments.
    Run: ./run test.c 6

    The complexity of the problem increases, because there are correct constructions of the form:
    1) "int (i);"
    2) "int
    i
    ;"
...
int main(int argc, char **argv)
{
    ...
    while(tmp_c != EOF)
    {
        ...
        if(tmp_c == '/')
        {
            ...
            /* If a 'classic' comment is encountered: */
            else if(tmp_c == '*')
            {
